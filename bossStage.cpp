...
HRESULT bossStage::init()
{
	...
	_enemyManager->init();
	_doorCount = 0;
	_enemyManager->setEnemy(MONSTRO, 6, 3);
...
HRESULT bossStage::init(isaac* _isaac, float ix, float iy, float tx, float ty)
{
	return S_OK;
}
...
//눙물과 벽 충돌
void bossStage::tearWallCollision()
{
	for (_viTear = _vTear->begin(); _viTear != _vTear->end(); )
	{
		// 눈물 벽 충돌
...
// 아이작과 적 충돌
void bossStage::isaacEnemyCollision()
{
	RECT rc = _isaac->getRect();
	for (_viEnemy = _vEnemy->begin(); _viEnemy != _vEnemy->end(); ++_viEnemy)
	{
	...
	for (_viETear = _vETear->begin(); _viETear != _vETear->end(); )
	{
		if (IntersectRect(&_temp, &rc, &_viETear->rc) && _viETear->z < 80)
		{		
				if (_isaac->getIst() != STDEATH && _isaac->getDefenceTime() == 0) 
				{
...
void bossStage::EnemyTearCollision()
{
	for (_viEnemy = _vEnemy->begin(); _viEnemy != _vEnemy->end(); ++_viEnemy)
	{
		RECT eRc = (*_viEnemy)->getRect();
		if ((*_viEnemy)->getZ() > -100)
		for (_viTear = _vTear->begin(); _viTear != _vTear->end();)
		{
...
void bossStage::stageClear()
{
	...
	_isClear = true;
	EFFECTMANAGER->play("item_poof", WINSIZEX / 2, WINSIZEY / 2 - 30, WINSIZEY / 2 + 50);
	_itemManager->setItem(6, 3);
	

}

void bossStage::doorMove()
{
	// 문열기
	for (int i = 0; i < 4; i++)
	{
...
// 카메라 세팅
void bossStage::moveCameraSetting(float increaseX, float increaseY, float targetX, float targetY)
{
