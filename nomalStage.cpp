...
HRESULT nomalStage::init()
{
	...
	{
	case 0:	
		_enemyManager->setEnemy(SUCKER, 0, 0);
		_enemyManager->setEnemy(SUCKER, 0, 6);
		_enemyManager->setEnemy(SUCKER, 12, 6);
		_enemyManager->setEnemy(SUCKER, 12, 0);
		_enemyManager->setEnemy(HORF, 6, 3);
		_grid->makeRocks(0, 0, 0, 2);
		_grid->makeRocks(0, 4, 0, 6);
		_grid->makeRocks(12, 0, 12, 2);
		_grid->makeRocks(12, 4, 12, 6);

		_grid->makeRocks(5, 2, 5, 4);
		_grid->makeRocks(7, 2, 7, 4);
		_grid->makeRock(5, 3);
		_grid->makeRock(7, 3);
		break;
	case 1:	
		_enemyManager->setEnemy(CLOTTY, 3, 1);
		_enemyManager->setEnemy(CLOTTY, 8, 5);
		_enemyManager->setEnemy(SUCKER, 0, 0);
		_enemyManager->setEnemy(SUCKER, 12, 6);
		_itemManager->setBomb(4, 3);
		_itemManager->setCoin(5, 3);
		_itemManager->setKey(7, 3);
		_itemManager->setHeart(8, 3);

		_grid->makeRocks(3, 4, 9, 4);
		_grid->makeRocks(3, 2, 9, 2);
		_grid->makeRock(3, 3);
		_grid->makeRock(9, 3);
		break;
	case 2:	
		_itemManager->setItem(6, 2);
		_enemyManager->setEnemy(CLOTTY, 3, 1);
		_enemyManager->setEnemy(CLOTTY, 8, 1);
		_enemyManager->setEnemy(CLOTTY, 3, 5);
		_enemyManager->setEnemy(CLOTTY, 8, 5);
		break;
	case 3:
		_enemyManager->setEnemy(HORF, 6, 2);
		_enemyManager->setEnemy(HORF, 6, 3);
		_enemyManager->setEnemy(HORF, 6, 4);
		_itemManager->setBomb(0, 0);
		_itemManager->setBomb(0, 6);
		_itemManager->setBomb(12, 0);
		_itemManager->setBomb(12, 6);
		_grid->makeRocks(5, 0, 5, 6);
		_grid->makeRocks(7, 0, 7, 6);
		break;
	default:
		break;
	}
...
HRESULT nomalStage::init(isaac * _isaac, float ix, float iy, float tx, float ty)
{
	return S_OK;
}

void nomalStage::release()
{
}

void nomalStage::update()
{
	// 맵 처음오면 세팅
	if (!_settingFlag)
	{
...
//눙물과 벽 충돌
void nomalStage::tearWallCollision()
{
	for (_viTear = _vTear->begin(); _viTear != _vTear->end(); )
	{
		// 눈물 벽 충돌
...
// 아이작과 적 충돌
void nomalStage::isaacEnemyCollision()
{
	RECT rc = _isaac->getRect();
	for (_viEnemy = _vEnemy->begin(); _viEnemy != _vEnemy->end(); ++_viEnemy)
	{
...
void nomalStage::EnemyTearCollision()
{
	for (_viEnemy = _vEnemy->begin(); _viEnemy != _vEnemy->end(); ++_viEnemy)
	{
		RECT eRc = (*_viEnemy)->getRect();
...
void nomalStage::doorMove()
{
	...
	else if (RENDERMANAGER->getCameraY() == -WINSIZEY)
	{
		...
		SCENEMANAGER->setScene("스테이지2");
	}
}

// 카메라 세팅
void nomalStage::moveCameraSetting(float increaseX, float increaseY, float targetX, float targetY)
{
