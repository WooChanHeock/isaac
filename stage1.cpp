...
HRESULT stage1::init()
{
	...
	case 2:
		...
		_enemyManager->setEnemy(HORF, 1, 6);
		_enemyManager->setEnemy(HORF, 11, 6);
		break;
	default:
		break;
	}
...
HRESULT stage1::init(isaac* _isaac, float ix, float iy, float tx, float ty)
{
	return S_OK;
}
...
// 아이작과 적 충돌
void stage1::isaacEnemyCollision()
{
	RECT rc = _isaac->getRect();
	for (_viEnemy = _vEnemy->begin(); _viEnemy != _vEnemy->end(); ++_viEnemy)
	{
...
void stage1::EnemyTearCollision()
{
	for (_viEnemy = _vEnemy->begin(); _viEnemy != _vEnemy->end(); ++_viEnemy)
	{
		RECT eRc = (*_viEnemy)->getRect();
...
void stage1::doorMove()
{
	...
	else if (RENDERMANAGER->getCameraX() == -WINSIZEX) // 오른쪽
	{
		_vTear->clear();
		_vTear->resize(0);
		_isaac->setX(224);
...
// 카메라 세팅
void stage1::moveCameraSetting(float increaseX, float increaseY, float targetX, float targetY)
{
