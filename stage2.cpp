...
HRESULT stage2::init()
{
	...
	{
	case 0:
		_enemyManager->setEnemy(DIP, 0, 0);
		_enemyManager->setEnemy(DIP, 1, 0);
		_enemyManager->setEnemy(DIP, 11, 0);
		_enemyManager->setEnemy(DIP, 12, 0);
		_enemyManager->setEnemy(DIP, 0, 6);
		_enemyManager->setEnemy(DIP, 1, 6);
		_enemyManager->setEnemy(DIP, 12, 6);
		_enemyManager->setEnemy(DIP, 11, 6);
		// 가운데 돌 원
	...
	case 1:
		...
		_grid->makeRocks(0, 1, 2, 1);
		_grid->makeRocks(0, 5, 2, 5);
		_grid->makeRocks(10, 5, 12, 5);
		_grid->makeRocks(10, 1, 12, 1);
		break;
	case 2:
		
		_enemyManager->setEnemy(HORF, 1, 0);
		_enemyManager->setEnemy(HORF, 11, 0);
		_enemyManager->setEnemy(SUCKER, 1, 6);
		_enemyManager->setEnemy(SUCKER, 11, 6);

		_grid->makeRocks(0,4,12,5);
		for (int i = 13; i != 0; i--) 
		{
			switch (RND->getFromIntTo(0,5))
			{
			case 0:
				_itemManager->setBomb(i - 1, 6);
				break;
			case 1:
				_itemManager->setCoin(i - 1, 6);

				break;
			case 2:
				_itemManager->setKey(i - 1, 6);

				break;
			case 3:
				_itemManager->setHeart(i - 1, 6);
				break;
			default:
				break;
			}
...
HRESULT stage2::init(isaac* _isaac, float ix, float iy, float tx, float ty)
{
	return S_OK;
}
...
// 아이작과 적 충돌
void stage2::isaacEnemyCollision()
{
	RECT rc = _isaac->getRect();
	for (_viEnemy = _vEnemy->begin(); _viEnemy != _vEnemy->end(); ++_viEnemy)
	{
...
void stage2::EnemyTearCollision()
{
	for (_viEnemy = _vEnemy->begin(); _viEnemy != _vEnemy->end(); ++_viEnemy)
	{
		RECT eRc = (*_viEnemy)->getRect();
...
// 카메라 세팅
void stage2::moveCameraSetting(float increaseX, float increaseY, float targetX, float targetY)
{
