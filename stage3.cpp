...
HRESULT stage3::init()
{
	...
	{
	case 0:
		_itemManager->setItem(3, 3);
		_itemManager->setItem(6, 3);
		_itemManager->setItem(9, 3);
		break;
	case 1:
		...
		break;
	case 2:
		_itemManager->setItem(8, 3);
		_itemManager->setItem(4, 3);
...
HRESULT stage3::init(isaac* _isaac, float ix, float iy, float tx, float ty)
{
	return S_OK;
}
...
// 아이작과 적 충돌
void stage3::isaacEnemyCollision()
{
	RECT rc = _isaac->getRect();
	for (_viEnemy = _vEnemy->begin(); _viEnemy != _vEnemy->end(); ++_viEnemy)
	{
...
void stage3::EnemyTearCollision()
{
	for (_viEnemy = _vEnemy->begin(); _viEnemy != _vEnemy->end(); ++_viEnemy)
	{
		RECT eRc = (*_viEnemy)->getRect();
...
// 카메라 세팅
void stage3::moveCameraSetting(float increaseX, float increaseY, float targetX, float targetY)
{
