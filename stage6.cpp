...
HRESULT stage6::init()
{
	...
	case 2:
		...
		_itemManager->setBomb(6, 2);
		_enemyManager->setEnemy(SUCKER, 1, 0);
		_enemyManager->setEnemy(SUCKER, 11, 0);
...
HRESULT stage6::init(isaac* _isaac, float ix, float iy, float tx, float ty)
{
	return S_OK;
}
...
// 아이작과 적 충돌
void stage6::isaacEnemyCollision()
{
	RECT rc = _isaac->getRect();
	for (_viEnemy = _vEnemy->begin(); _viEnemy != _vEnemy->end(); ++_viEnemy)
	{
...
void stage6::EnemyTearCollision()
{
	for (_viEnemy = _vEnemy->begin(); _viEnemy != _vEnemy->end(); ++_viEnemy)
	{
		RECT eRc = (*_viEnemy)->getRect();
...
// 카메라 세팅
void stage6::moveCameraSetting(float increaseX, float increaseY, float targetX, float targetY)
{
